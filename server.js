...
const server=http.createServer(async (req,res)=>{
  ...
  try {
    ...
    if(url.pathname==='/api/business/advertising-renewal'&&req.method==='POST'){
      ...
      const periodEnd=request.paymentStatus==='trial'||(!request.paidUntil&&request.trialUntil)?request.trialUntil:request.paidUntil,end=periodEnd?new Date(periodEnd):null,periodIsCurrent=Boolean(end&&!isNaN(end)&&end>new Date());
      if(request.status!=='approved')return json(res,409,{error:'Only an approved advertising listing can have its renewal changed.'});
      if(action==='stop'&&(!['paid','trial'].includes(request.paymentStatus)||!periodIsCurrent))return json(res,409,{error:'Only a currently active advertising period can be stopped from next month.'});
      if(action==='resume'&&!periodIsCurrent&&request.renewalStatus!=='stopped')return json(res,409,{error:'This advertising listing does not have a scheduled or completed stop to resume.'});
      ...
      if(action==='resume'){delete request.stoppedAt;if(periodIsCurrent){request.paymentStatus=periodEnd===request.trialUntil?'trial':'paid';request.listing='enabled'}else{request.paymentStatus='due';request.listing='disabled'}}
      reconcile(db);
      audit(db,{action:action==='stop'?'advertising.renewal_stop_scheduled':'advertising.renewal_resumed',entityType:'business_advertising',entityId:request.id,actor:business.name||business.email,before,after:{cancelAtPeriodEnd:request.cancelAtPeriodEnd,renewalStatus:request.renewalStatus,periodEnd:end&&!isNaN(end)?end.toISOString():null}});
      const when=end&&!isNaN(end)?end.toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'}):'';notify(db,`masjid:${request.masjidReference||request.masjid}`,action==='stop'?'Advertising will end':'Advertising renewal restored',action==='stop'?`${request.name} has chosen not to renew after ${when}. The listing remains live until then.`:periodIsCurrent?`${request.name} restored renewal for its listing. The current advert remains live.`:`${request.name} restarted renewal and must clear the new monthly invoice before the advert returns.`,'masjid-listings',`advert-renewal-${request.id}-${action}-${request.renewalChangedAt}`);
      await save(db);return json(res,200,{ok:true,cancelAtPeriodEnd:request.cancelAtPeriodEnd,renewalStatus:request.renewalStatus,listing:request.listing,paymentStatus:request.paymentStatus,periodEnd:end&&!isNaN(end)?end.toISOString():null});
    }
    ...
    if (url.pathname==='/api/state' && req.method==='GET') { const supplied=Boolean(req.headers['x-masjidpoint-session']);const session=readSession(req);if(supplied&&!session)return json(res,401,{error:'Your session has expired. Please sign in again.'});const db=await load(),before=JSON.stringify(db);reconcile(db);if(JSON.stringify(db)!==before)await save(db);return json(res,200,stateForSession(db,session)); }
