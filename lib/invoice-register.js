...
(function (root) {
  ...
  function statusOf({ amount, paid, due, status }) {
    ...
    return (db.masjidPointFinance?.accounts || []).flatMap(account =>
      (account.invoices || []).map(invoice => {
        ...
        return {
          ...
          status: statusOf({ amount, paid, due: invoice.due, status: invoice.status }),
          route: null,
  ...
  function fromShop(db) {
    ...
    return (db.masjidPointShopOrders || []).filter(order => order.invoiceNumber).map(order => {
      ...
      return {
        ...
        status: statusOf({ amount, paid, due, status: order.invoiceStatus }),
        route: method,
