// Shared definitions for the three ways a customer can receive a mosque shop order.
// Loaded by the public shop, the admin order desk and the mosque portal so the
// labels, payment expectations and status chains never drift apart.
(function (root) {
  const METHODS = {
    collect_pay_now: {
      key: 'collect_pay_now',
  ...
  // Legacy orders predate fulfilmentMethod: infer it from how they were paid.
  ...
  // The next status an order can move to, or null when it has reached the end of its chain.
  function nextStatus(order) {
    const chain = methodOf(order).statuses, at = chain.indexOf(order?.status);
  ...
  const api = { METHODS, ORDER, DEFAULT_SETTINGS, methodOf, settingsOf, enabledFor, deliveryFeeOf, nextStatus, paymentLabel, addressLines };
  if (typeof module !== 'undefined' && module.exports) module.exports = api;
  root.ShopFulfilment = api;
