...
(async()=>{
 ...
 await connect();await cdp('Page.enable');await cdp('Runtime.enable');await cdp('Page.bringToFront');
 ...
 const mobileStats=await ev(`(()=>{const items=[...document.querySelectorAll('#trust-row>div')],track=document.querySelector('#promise-track'),cards=[...track.children],dots=document.querySelector('.promise-dots'),dot=dots.querySelector('button'),businessGrid=document.querySelector('#business-grid'),businessCards=[...businessGrid.children].filter(card=>!card.hidden),businessDots=[...document.querySelectorAll('.business-carousel-dots button')],headingSpans=[...document.querySelectorAll('#discover h2>span')],topLink=document.querySelector('#discover .section-heading>.arrow-link'),bottomLink=document.querySelector('#business-grid-more');return {columns:getComputedStyle(document.querySelector('#trust-row')).gridTemplateColumns.split(' ').length,tops:items.map(item=>Math.round(item.getBoundingClientRect().top)),promiseDisplay:getComputedStyle(track).display,promiseScrollable:track.scrollWidth>track.clientWidth,nextCardPeeks:cards[1].getBoundingClientRect().left<innerWidth,dotsVisible:getComputedStyle(dots).display!=='none',dotHeight:Math.round(dot.getBoundingClientRect().height),categoriesRemoved:!document.querySelector('#category-chips'),headingLines:headingSpans.length,headingSpanDisplay:headingSpans.map(span=>getComputedStyle(span).display),visibleBusinesses:businessCards.length,businessScrollable:businessGrid.scrollWidth>businessGrid.clientWidth,businessDots:businessDots.length,businessDotHeight:businessDots[0]?Math.round(businessDots[0].getBoundingClientRect().height):0,topLinkHidden:getComputedStyle(topLink).display==='none',bottomLinkWidth:bottomLink?Math.round(bottomLink.getBoundingClientRect().width):0,bottomLinkText:bottomLink?.textContent.trim(),autoEligible:!matchMedia('(prefers-reduced-motion: reduce)').matches&&!document.hidden}})()`);
 assert(mobileStats.columns===3&&new Set(mobileStats.tops).size===1,`Mobile impact figures wrapped: ${JSON.stringify(mobileStats)}`);
 ...
 const masjidCarousel=await ev(`(()=>{const section=document.querySelector('#masjids'),grid=document.querySelector('#masjid-grid'),cards=[...grid.children].filter(card=>!card.hidden),row=document.querySelector('.masjid-carousel-controls'),dots=document.querySelector('.masjid-carousel-dots'),link=document.querySelector('#masjid-grid-more'),heading=[...section.querySelectorAll('h2>span')],topLink=section.querySelector('.section-heading>.arrow-link');if(!row||!dots||!link)return {row:!!row,dotsElement:!!dots,link:!!link,totalCards:grid.children.length,visible:cards.length};const r=row.getBoundingClientRect(),d=dots.getBoundingClientRect(),l=link.getBoundingClientRect();return {visible:cards.length,scrollable:grid.scrollWidth>grid.clientWidth,dots:dots.children.length,dotHeight:Math.round(dots.firstElementChild.getBoundingClientRect().height),headingLines:heading.length,headingBlocks:heading.every(span=>getComputedStyle(span).display==='block'),topLinkHidden:getComputedStyle(topLink).display==='none',linkText:link.textContent.trim(),rowCenter:Math.round(r.left+r.width/2),dotsCenter:Math.round(d.left+d.width/2),linkLeft:Math.round(l.left),dotsLeft:Math.round(d.left)}})()`);
 assert(masjidCarousel&&masjidCarousel.visible>0&&masjidCarousel.visible<=6&&masjidCarousel.scrollable&&masjidCarousel.dots===masjidCarousel.visible&&masjidCarousel.dotHeight<=8&&masjidCarousel.headingLines===2&&masjidCarousel.headingBlocks&&masjidCarousel.topLinkHidden&&masjidCarousel.linkText==='View all →'&&Math.abs(masjidCarousel.rowCenter-masjidCarousel.dotsCenter)<=2&&masjidCarousel.linkLeft>masjidCarousel.dotsLeft,`Mobile masjid section does not match the business carousel: ${JSON.stringify(masjidCarousel)}`);
 const mobileShop=await ev(`(()=>{const heading=document.querySelector('#shop h2'),label=document.querySelector('.shop-products-label'),art=document.querySelector('#shop-preview'),cta=document.querySelector('.shop-mobile-cta'),ways=[...document.querySelectorAll('.shop-banner-ways em')],style=getComputedStyle(heading),h=heading.getBoundingClientRect(),a=art.getBoundingClientRect(),c=cta.getBoundingClientRect();return {headingLines:Math.round(h.height/parseFloat(style.lineHeight)),labelVisible:getComputedStyle(label).display!=='none',label:label.textContent.trim(),ctaVisible:getComputedStyle(cta).display!=='none',ctaBelowProducts:c.top>=a.bottom,wayTops:ways.map(way=>Math.round(way.getBoundingClientRect().top))}})()`);
 assert(mobileShop.headingLines===2&&mobileShop.labelVisible&&mobileShop.label==='Popular newly added products from masjids'&&mobileShop.ctaVisible&&mobileShop.ctaBelowProducts&&new Set(mobileShop.wayTops).size===1,`Mobile shop hierarchy is incorrect: ${JSON.stringify(mobileShop)}`);
 await sleep(2700);
 const autoPromise=await ev(`({left:document.querySelector('#promise-track').scrollLeft,current:[...document.querySelectorAll('.promise-dots button')].findIndex(dot=>dot.getAttribute('aria-current')==='true')})`);
 if(mobileStats.autoEligible) assert(autoPromise.left>0&&autoPromise.current===1,`Mobile promise carousel did not advance automatically: ${JSON.stringify(autoPromise)}`);
 const autoBusiness=await ev(`({left:document.querySelector('#business-grid').scrollLeft,current:[...document.querySelectorAll('.business-carousel-dots button')].findIndex(dot=>dot.getAttribute('aria-current')==='true')})`);
 if(mobileStats.autoEligible&&mobileStats.visibleBusinesses>1) assert(autoBusiness.left>0&&autoBusiness.current===1,`Mobile business carousel did not advance automatically: ${JSON.stringify(autoBusiness)}`);
