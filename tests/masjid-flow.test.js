...
(async()=>{
  ...
  state=await get('/api/state');const loginAccount=state.masjidPointActivatedAccounts.find(a=>a.reference===reference);assert(loginAccount?.verified,'Account was not activated');const signIn=await fetch(`${base}/api/account/login`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,passwordHash:passwordHash(password)})});assert(signIn.status===200,'Password verification failed');const wrong=await fetch(`${base}/api/account/login`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,passwordHash:passwordHash(password+'x')})});assert(wrong.status===401,'A wrong password was accepted');const portal=await get('/masjid-portal.html');assert(portal.includes('portal-auth.js'),'Masjid portal is not protected by the login guard');
