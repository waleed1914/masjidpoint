(async function () {
  ...
  function periodEnd(request) { return request.paymentStatus === 'trial' ? request.trialUntil : request.paidUntil; }
  function dateText(value) { return new Date(value).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }); }
  function enhance() {
    const cards = [...host.querySelectorAll('.listing-row')];
    requests.forEach((request, index) => {
      const card = cards[index]; if (!card || card.querySelector('[data-renewal-request]')) return;
      const end = periodEnd(request), active = request.status === 'approved' && ['paid', 'trial'].includes(request.paymentStatus) && end && new Date(end) > new Date();
      if (!active) return;
      const actions = document.createElement('div'); actions.className = 'advert-renewal-actions';
      const scheduled = request.cancelAtPeriodEnd === true;
      actions.innerHTML = `${scheduled ? `<span>Ends ${dateText(end)} · no renewal charge</span>` : '<span>Renews monthly</span>'}<button type="button" data-renewal-request="${request.id}">${scheduled ? 'Keep advertising next month' : 'Stop from next month'}</button>`;
      card.querySelector('.listing-meta')?.appendChild(actions);
    });
  }
  function open(request) {
    selected = request; const stopping = !request.cancelAtPeriodEnd;
    ...
    document.querySelector('#renewal-date').textContent = dateText(periodEnd(request));
    document.querySelector('#renewal-message').textContent = stopping ? 'We will schedule this listing to finish at the end of its current paid period and will not raise the next monthly charge.' : 'The scheduled stop will be removed. The advert can continue into its next monthly period.';
  ...
  document.querySelector('#confirm-renewal').onclick = async event => {
    if (!selected) return; const button = event.currentTarget, stopping = !selected.cancelAtPeriodEnd, original = button.textContent;
